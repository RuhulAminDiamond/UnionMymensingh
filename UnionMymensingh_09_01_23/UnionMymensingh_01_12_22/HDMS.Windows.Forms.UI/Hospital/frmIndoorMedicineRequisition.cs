...
{
    public partial class frmIndoorMedicineRequisition : Form
    {

        bool unlocked = true;
        bool isPanelMinimized = true;

        public frmIndoorMedicineRequisition()
        {
            InitializeComponent();
        }
        ...
        {
            List<VMedicineRequisition> _reqList = new HospitalService().GetRequisitionListByUserByDate(username, dtp1.Value);
        ...
        private void FillIndentGrid(List<VMedicineRequisition> _hpReqList)
        {
            ...
            foreach (VMedicineRequisition item in _hpReqList)
            {
                ...
                row.CreateCells(dgRequistions, item.RequisitionId, item.CabinNo, item.RequisitionStatus, item.RequisitionBy);
        ...
        private async void btnSave_Click(object sender, EventArgs e)
        {
            if (dgRequisition.Rows.Count > 0)
            {
                ...
                HpMedicineRequisition _newRequisition = await new HospitalService().CreateNewIPdPhRequistion(_hpMReq, _reqDetailsList);
                if (_newRequisition.RequisitionId > 0)
                {
