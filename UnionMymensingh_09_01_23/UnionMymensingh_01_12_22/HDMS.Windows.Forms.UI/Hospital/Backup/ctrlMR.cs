        ...
        private void dgPatient_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (dgPatient.SelectedRows.Count != 0)
            {
                    ...
                    if (_cInfoList.Count > 0)
                    {
                        ...
                        foreach (var item in _cInfoList)
                        {
                            ...
                            if (isWithin24Hours)
                            {
                                    ...
                                    _daysInCabin = new CalculateCabinCharge().DaysInHospital(item.AccomodateDate, item.AccomodateTime, item.ReleaseDate, item.ReleaseTime, 0, false, isSandwichedhours_Greater_Than_GracePeriod_Considerable);

                                   

                                    isCurrentOccupiedCabinWithin24HoursOfAdmission = new CalculateCabinCharge().IsWithin24Hours(_admissionDate, _admissionTime, _currentAccomInfo.AccomodateDate, _currentAccomInfo.AccomodateTime);
                            ...
                            else
                            {
                                    ...
                                    else
                                    {
                                        ...
                                        _daysInCabin = new CalculateCabinCharge().DaysInHospital(item.AccomodateDate, item.AccomodateTime, item.ReleaseDate, item.ReleaseTime, _gracePeriod, false, isSandwichedhours_Greater_Than_GracePeriod_Considerable);
                                    }
                ...
                if (_cabinChargeListWithin24Hours.Count > 0)
                {
                        ...
                        {
                            _cabinWithin24hours = _cabinChargeListWithin24Hours.OrderBy(x => x.Rate).FirstOrDefault();
                        }
                    ...
                    {
                          IsOccupiedCabinTheLeastBill = true;
                            if (String.IsNullOrEmpty(_billJustification))
                            {
                ...
                if (_pAccomInfo != null)
                {
                    ...
                    if (IsOccupiedCabinFoundForTransferredWithin24Hours && IsOccupiedCabinTheLeastBill)
                    {
                            _daysInCabin = 1;//For First 24 hours + Grace Period
                            _tempAccomdate = new CalculateCabinCharge().GetAccomodateDateAddingGracePeriod(_pAccomInfo.AccomodateDate, _pAccomInfo.AccomodateTime, _gracePeriod+24); //24 for first 24 hours
                    ...
                    else if (IsOccupiedCabinFoundForTransferredWithin24Hours && !IsOccupiedCabinTheLeastBill)
                    {
                            ...
                            _daysInCabin = new CalculateCabinCharge().DaysInHospital(_tempAccomdate.Date, _tempAccomdate.ToString("hh:mm tt"), Utils.GetServerDateAndTime(), Utils.GetServerDateAndTime().ToString("hh:mm tt"), 0, false, isSandwichedhours_Greater_Than_GracePeriod_Considerable);
                    ...
                    else
                    {
                            ...
                            {
                                _daysInCabin = new CalculateCabinCharge().DaysInHospital(_pAccomInfo.AccomodateDate, _pAccomInfo.AccomodateTime, Utils.GetServerDateAndTime(), Utils.GetServerDateAndTime().ToString("hh:mm tt"), _gracePeriod, false, isSandwichedhours_Greater_Than_GracePeriod_Considerable);
                            }
                            else
                            {
                                ...
                                if (_timeDistanceInhours > 24)
                                {
                                    ...
                                    _daysInCabin =  1; // 1 for First 24 hours + Grace Period
                                     ...
                                     double _sandwichedTime = new CalculateCabinCharge().GetTimeDistanceInHours(_tempAccomdate.Date, _tempAccomdate.ToString("hh:mm tt"), _tempAccomdate.Date.AddDays(1), "12:01 AM");
                ...
                Thread.Sleep(100);
                List<VMHpFinalBill> finalBillItems = new HpFinancialService().GetHpFinalBillItems(_pInfo.AdmissionId);
        ...
        private void ViewAdvanceReceipt(long _hbillId, string cabinNo)
        {
            ...
            if (_pInfo != null)
            {
                ...
                if (Configuration.ORG_CODE == "1")
                {
                    ...
                    _moneyReceipt.SetParameterValue("ContactNo", ComapnyDetail.BranchAddress1_ContactNo + ", " + ComapnyDetail.EmailAddress);
