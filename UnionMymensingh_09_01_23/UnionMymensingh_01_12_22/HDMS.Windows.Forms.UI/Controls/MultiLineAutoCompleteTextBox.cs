...
{
    public class MultiLineAutoCompleteTextBox : TextBox
    {
        ...
        private void ShowListBox()
        {
            if (!_isAdded)
            {
                ...
                _listBox.Font = new Font("Microsoft Sans Serif", 11f, FontStyle.Regular);
                parentForm.Controls.Add(_listBox);
                Point positionOnForm = parentForm.PointToClient(Parent.PointToScreen(Location));
                _listBox.Left = positionOnForm.X+Width;
                _listBox.Top = positionOnForm.Y;
                _isAdded = true;
            }
                ...
                if (matches.Length > 0)
                {
                    ...
                    using (Graphics graphics = _listBox.CreateGraphics())
                    {
                        for (int i = 0; i < _listBox.Items.Count; i++)
                        {
                            ...
                            _listBox.Width = (_listBox.Width < itemWidth) ? itemWidth+10 : Width+10; ;
                        }
                    }
                    _listBox.EndUpdate();
                }
