...
{
    public partial class PhIPDProductSalesControl : UserControl
    {
        bool unlocked = true;
        ...
        public PhIPDProductSalesControl()
        {
            InitializeComponent();
        }
        ...
        private void UpdateFont()
        {
            ...
            foreach (DataGridViewColumn c in dgRequistions.Columns)
            {
                c.DefaultCellStyle.Font = new Font("Segoe UI", 14.5F, GraphicsUnit.Pixel);
            ...
            dgRequistions.Font = new Font("Segoe UI", 14.5F, GraphicsUnit.Pixel);
        ...
        private void HideAllDefaultHidden()
        {
            ...
            ctlReplaceProductSearchControl.Visible = false;
        }
        ...
        private async void PhProductSalesControl_Load(object sender, EventArgs e)
        {
            ...
            LoadRequisitions("0", _moutLet.OutLetId);
        ...
        private void ctrlProductSearchByGeneric_ItemSelected(SearchResultListControlForPharmacy<VWPhProductList> sender, VWPhProductList item)
        {
            txtReplaceby.Text = item.BrandName;
            txtReplaceby.Tag = item;
            sender.Visible = false;
        ...
        private void LoadRequisitions(string floorId,int _outletId)
        {
            int _floorId = 0;
            int.TryParse(floorId, out _floorId);

            List<VMedicineRequisition> _hpReqList = new HospitalService().GetHpPendingRequisitionList(_floorId, _outletId);
            FillIndentGrid(_hpReqList);
        ...
        private void FillIndentGrid(List<VMedicineRequisition> _hpReqList)
        {
            ...
            foreach (VMedicineRequisition item in _hpReqList)
            {
                ...
                row.CreateCells(dgRequistions, item.RequisitionId, item.CabinNo, item.RequisitionBy, item.RequisitionStatus);
        ...
        private void UpdateStockOnUILabel(VWPhProductListForSale item)
        {
            ...
            label33.Text = _st.PurchaseRate.ToString();
        ...
        private void FillItemGrid()
        {
            ...
            foreach (PhSelectedProductsToSaleOrReceiveOrOrder item in _SelectedItems)
            {
                if (item.Qty > 0)
                {
                    ...
                    row.Tag = item;
                    row.CreateCells(dgSales, item.Id, item.Name, item.BatchNo, item.ExpireDate.ToString("dd/MM/yyyy"), item.SRate, item.Qty, item.Total, item.PTotal);
        ...
        private void CalculateAmount()
        {
            ...
            _total=Math.Round(dgSales.Rows.Cast<DataGridViewRow>()
                .Sum(t => Convert.ToDouble(t.Cells[6].Value)),2);
                ...
                if (dgSales.Rows.Count > 0)
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (_outLet.OutLetId == 0)
                        {
                            ...
                            if (_dueTk > 0)
                            {
                                if(txtAdmissionNo.Tag==null && txtMember.Tag==null && txtRequisitionNo.Tag==null) {
                                    MessageBox.Show("It's due sale.Admission No/Member No/Requisition No required.");
                                     btnSave.Enabled = true;
                                     btnSave.Text = "Save";
                                     return;
                                }
                        ...
                        if (txtAdmissionNo.Tag==null && txtMember.Tag==null && txtEmployeeNo.Tag == null)
                        {
                        ...
                        //{
                           ...
                           if (_admissionNo > 0)
                           {
                                    HpMedicineRequisition _Mreq = (HpMedicineRequisition)txtRequisitionNo.Tag;
                                    HospitalPatientInfo _pInfo = (HospitalPatientInfo)txtAdmissionNo.Tag;
                                    voucherObj.HospitalPatientInfo = _pInfo;
                                    voucherObj.hpMedicineRequisition = _Mreq;
                                    voucherObj.IPDRequisitionDeliveryStatus = IPDRequisitionDeliveryStatus;

                                    if (_dueTk > 0 && MainForm.orgSetting.IsIntegratedAccountInAction) {

                                        voucherObj.ReceivableAccId = 1963;
                        ...
                        {
                            PhInvoice _Invoice = await new PhFinancialService().SaveAndCommitNewSale(pi, _invDetailsList, voucherObj, MainForm.orgSetting.IsIntegratedAccountInAction);
                            ...
                            if (_Invoice.InvoiceId > 0)
                            {
                                ...
                                if (IsInddorSale)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (cbInvoiceType.Tag != null && Convert.ToInt32(cbInvoiceType.Tag) == 3)
                                        {
                                            PrintIndoorToOTSaleInvoice(invocieId);
                                        }
                                        else if (Convert.ToInt32(cmbOutlet.SelectedValue) == 3)
                                        {
                                            PrintIndoorOTSaleInvoice(invocieId);
                                        }
                                        else
                                        {
                                            PrintIndoorInvoice(invocieId);
                                        }
                            ...
                            LoadRequisitions(cmbFloor.SelectedValue.ToString(), _outLet.OutLetId);
        ...
        private void PrintIndoorOTSaleInvoice(long invocieId)
        {
            int _index = 0;
            bool isDue = false;
            ...
            DataSet ds = new PhReportingService().GetIndoorOTInvoiceDataByinvocieId(invocieId);

            IndoorOTInvoivce _rpt = new IndoorOTInvoivce();
            ...
            _rpt.SetParameterValue("CabinNo", txtCabin.Text);
        ...
        private void PrintIndoorToOTSaleInvoice(long invocieId)
        {
            int _index = 1;
            bool isDue = false;
            bool isDiscounted = false;
            ...
            DataSet ds = new PhReportingService().GetIndoorToOTInvoiceDataByinvocieId(invocieId);
            ...
            {
                if (litem.TransactionType != "TotalAmount")
                {
            ...
            for (int _count = _index + 1; _count <= 4; _count++)
            {
            ...
            _rpt.SetParameterValue("RequisitionNo", _Invoice.RequisitionNo.ToString());
            _rpt.SetParameterValue("CabinNo", txtCabin.Text);
            _rpt.SetParameterValue("InvoiceHeaderText", "OT INVOICE");
        ...
        private void PrintIndoorInvoice(long invocieId)
        {
            int _index = 1;
            bool isDue = false;
            bool isDiscounted = false;
            ...
            _rpt.SetParameterValue("PayStatus", "DUE");
            _rpt.SetParameterValue("BillNo", _pInvoice.BillNo.ToString());
            _rpt.SetParameterValue("RequisitionNo", _pInvoice.RequisitionNo.ToString());
            _rpt.SetParameterValue("CabinNo", txtCabin.Text);
            _rpt.SetParameterValue("InvoiceHeaderText", "IPD INVOICE");
        ...
        private void calculateDAndR()
        {
            ...
            totalReceived = received + cardormobreceive;
        ...
        private void cbInvoiceType_TextChanged(object sender, EventArgs e)
        {
            ...
            else if(cbInvoiceType.SelectedIndex==2)
            {
        ...
        private void txtAdmission_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                HpPatientAccomodationInfo accomInfo = new HospitalCabinBedService().GetOccupiedCabinByPatient(_pInfo.AdmissionId);
                if (accomInfo != null)
                {
                    txtCabin.Text = new HospitalCabinBedService().GetCabinInfoId(accomInfo.CabinId).CabinNo;
                }
                ...
                if (_record != null)
                {
                    ...
                    txtCustomerName.Text = _record.FullName;
                 
                }
                txtAdmissionNo.Text = _pInfo.BillNo.ToString();
        ...
        private void btnPrevId_Click(object sender, EventArgs e)
        {
            ...
            {
                _Invoice = new PhProductService().GetPhIPDLastestInvoice();
            }
            else
            {
                long _billNo = 0;
            ...
            if (_Invoice != null)
            {
                ...
                {
                    txtAdmissionNo.Text = _Invoice.AdmissionNo.ToString();
                    HospitalPatientInfo _pInfo = new HospitalService().GetHospitalPatientByBillNo(_Invoice.AdmissionNo);
                    if (_pInfo != null)
                    {
                        HpPatientAccomodationInfo _pAccomInfo = new HospitalService().GetHpPatientCurrentAccomodation(_pInfo.AdmissionId);
                        if (_pAccomInfo != null)
                        {
                            CabinInfo _cInfo = new HospitalCabinBedService().GetCabinInfoId(_pAccomInfo.CabinId);
                            txtCabin.Text = _cInfo.CabinNo;
                        }
                        else
                        {
                            txtCabin.Text = "";
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (_admissionNo > 0)
            {
                HospitalPatientInfo _hpInfo = new HospitalService().GetHospitalPatientByBillNoAny(_admissionNo);
                if (_hpInfo != null)
                {
                    PrintIndoorInvoice(_Invoice.InvoiceId);
                }
        ...
        private void txtAdmissionNo_Leave(object sender, EventArgs e)
        {
            ...
            HospitalPatientInfo _hpInfo = new HospitalService().GetHospitalPatientByBillNo(_admissionNo);
        ...
        private void dgRequistions_RowHeaderMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            txtRequisitionNo.Text = dgRequistions.Rows[e.RowIndex].Cells["RequisitionNo"].Value.ToString();
        ...
        private void LoadRequitedProducts(string _reqNo)
        {
            ...
            HpMedicineRequisition _MedReq = new HospitalService().GetHpMedicineRequisitionByReqId(_requisitionNo);
            txtRequisitionNo.Tag = _MedReq;

            if (_MedReq == null) return;

            if (_MedReq.RequisitionType == RequisitionTypeEnum.ForPatient.ToString())
            {
                HospitalPatientInfo _hpInfo = new HospitalService().GetHospitalPatientInfoById(_MedReq.AdmissionId);

                if (_hpInfo != null)
                {
                    txtAdmissionNo.Text = _hpInfo.BillNo.ToString();
                    txtCustomerName.Text = new RegRecordService().GetRegRecordByRegNo(_hpInfo.RegNo).FullName;
                    Doctor _doc = new DoctorService().GetDoctorById(_hpInfo.AssignDoctorId);
                    if (_doc != null)
                    {
                        lblAssignedDoc.Text = "Assigned Doctor:" + _doc.Name;
                    }

                    HpPatientAccomodationInfo _accomInfo = new HospitalCabinBedService().GetOccupiedCabinByPatient(_hpInfo.AdmissionId);

                    if (_accomInfo.CabinId>0)
                    {
                        txtCabin.Text = new HospitalCabinBedService().GetCabinInfoId(_accomInfo.CabinId).CabinNo;
                    }
                    else
                    {
                        txtCabin.Text = "";
            ...
            if (_MedReq != null)
            {
                List<VMRequisitionList> _mReqDetail = new HospitalService().GetHpMedicineRequisitionDetailByReqId(_MedReq.RequisitionId, Convert.ToInt32(cmbOutlet.SelectedValue));
        ...
        private void FillReqProductGrid(List<VMRequisitionList> _mReqDetail)
        {
            ...
            foreach (VMRequisitionList item in _mReqDetail)
            {
                ...
                row.DefaultCellStyle.ForeColor = Color.Black;
                row.Tag = item;
                row.CreateCells(dgRequisitionList, item.ProductId, item.BrandName, item.ReqQty, item.StockAvailable, item.IsStockShort,item.ItemDeliveryStatus,item.ReplaceRemarks);
        ...
        private void dgRequisitionList_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dgRequisitionList.Rows)
            {
                ...
                new PhProductService().PopulateSelectedItemDataForSaleAgainstRequisitionFromNS(_SelectedItems, selectedItem); //NS: Nurse Station
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            if (btnCancel.Tag != null)
            {
                ...
                {
                    long _reqNo = 0;
                    long.TryParse(btnCancel.Tag.ToString(), out _reqNo);
                    
                      
                    new PhProductService().CancelMedicineRequisition(_reqNo);
        ...
        private void SetTogglePositionForPanel(bool _IspanleMinimized)
        {
            ...
            {
                lblRequisitionPanel.Location = new Point(1230, 66);
            ...
            {
                lblRequisitionPanel.Location = new Point(10, 66);
        ...
        private void btnRefresh_Click(object sender, EventArgs e)
        {
            LoadFloors();
            MedicineOutlet _OutLet = (MedicineOutlet)cmbOutlet.SelectedItem;
            LoadRequisitions("0", _OutLet.OutLetId);
        }
        ...
        private void dgRequistions_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            cbInvoiceType.Tag = _mreq.OutletId;
            ...
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (txtReplaceby.Tag != null)
                {
                    if (dgRequisitionList.Tag != null)
                    {
                        ...
                        List<VMRequisitionList> _reqList = (List<VMRequisitionList>)dgRequisitionList.Tag;

                       VMRequisitionList _requisition = _reqList.Where(x => x.GenericId == _PhProduct.GenericId).FirstOrDefault();
                        ...
                        {
                            HpMedicineRequisitionDetail _reqDetail = new PhProductService().GetHpMedicineRequisitionDetail(_requisition.RequisitionId, _requisition.ProductId);

                            _reqDetail.ProductId = _PhProduct.ProductId;
                            _reqDetail.ReplaceRemarks = "Replaced of: " + _requisition.BrandName;

                            new PhProductService().UpdateHpMedicineRequisitionDetail(_reqDetail);

                            List<VMRequisitionList> _mReqDetail = new HospitalService().GetHpMedicineRequisitionDetailByReqId(_requisition.RequisitionId, Convert.ToInt32(cmbOutlet.SelectedValue));
                            //dgRequisitionList.Visible = true;
                            FillReqProductGrid(_mReqDetail);
