...
{
    public partial class PatientEntryControl02032019 : UserControl
    {
        bool isLoaded = false;
        ...
        bool isReportDeliveryWeekEndSchedule = false;
        ...
        //double totalMediaCommTk = 0;


        public LoginUser loggedInUser { get; set; }

        DateTime _firsthalf, _secondHalf, _thirdHalf, _4thhalf, _5thHalf, _6thHalf, _7thhalf, _8thHalf;
        string _1stDeliveryTime, _2ndDeliveryTime, _3rdDeliveryTime, _4thDeliveryTime, _5thDeliveryTime, _6thDeliveryTime, _7thDeliveryTime, _8thDeliveryTime;


        public PatientEntryControl02032019()
        {
        ...
        private void FillTestGrid()
        {
            ...
            _SelectedTests = _SelectedTests.OrderBy(x => x.DisplayOrder).ToList();
        ...
        private void CalculateTestAmount()
        {
            ...
            double _due = 0, _paid = 0, _discount = 0;
        ...
        private async void PatientEntryControl02032019_Load(object sender, EventArgs e)
        {
            btnMaxView_Click(null, null);
            ...
            ctlPatientList.Location = new Point(txtAdmissionNo.Location.X, txtAdmissionNo.Location.Y + txtAdmissionNo.Height);
            ctlPatientList.ItemSelected += ctlPatientList_ItemSelected;

            ctlMarketing.Location = new Point(txtMarketing.Location.X, txtMarketing.Location.Y);
            ctlMarketing.ItemSelected += ctlMarketing_ItemSelected;
            ...
            await LoadReportDeliveryTiming();
        ...
        private void ctlMarketing_ItemSelected(SearchResultListControl<BusinessMedia> sender, BusinessMedia item)
        {
            txtMarketing.Text = item.Name;
            txtMarketing.Tag = item.MediaId;
            txtMarketing.Focus();
        ...
        {
            txtAdmissionNo.Text = item.AdmissionId.ToString();
            txtPatientName.Text = item.Name;
            txtCabin.Text = item.BedCabinNo;


            txtAdmissionNo.Tag = item;
        ...
        {
            List<VMDiagPatientBasicInfo> _lisPatientInfo = await new PatientService().GetDiagnosticPatienInfo(dtpdiagfrm.Value, dtpdiagto.Value);
            FillListGrid(_lisPatientInfo);
            return _lisPatientInfo;
        }

        private void FillListGrid(List<VMDiagPatientBasicInfo> _lisPatientInfo)
        {
            dgPatient.SuspendLayout();
            dgPatient.DefaultCellStyle.Font = new Font("Arial", 15, GraphicsUnit.Pixel);
            dgPatient.RowTemplate.DefaultCellStyle.Padding = new Padding(0, 0, 0, 0);
            dgPatient.Rows.Clear();
            foreach (VMDiagPatientBasicInfo item in _lisPatientInfo)
            {
                DataGridViewRow row = new DataGridViewRow();
                row.Tag = item;
                row.Height = 35;
                row.CreateCells(dgPatient, item.PatientId, item.BillNo, item.PatientName,  item.Ageyear, item.MobileNo, item.Name, item.MediaName, item.ArearOrThana, item.EntryDate, item.EntryTime, item.DiscountCareOf, item.UserName);
        ...
        private async Task<bool> LoadReportDeliveryTiming()
        {
            ReportDeliveryTimingMaster selectedtm = await new TestService().GetSelectedReportDeliveryTimeMaster();
            if (selectedtm != null)
            {
                List<ReportDeliveryTimingDetail> td = await new TestService().GetSelectedReportDeliveryTimingDetail(selectedtm.RDTMId);
                int count = 1;
                foreach (var item in td)
                {
                    if (count == 1) { _firsthalf = Convert.ToDateTime(item.EntryTime); _1stDeliveryTime = item.DeliveryTime; }
                    if (count == 2) { _secondHalf = Convert.ToDateTime(item.EntryTime); _2ndDeliveryTime = item.DeliveryTime; }
                    if (count == 3) { _thirdHalf = Convert.ToDateTime(item.EntryTime); _3rdDeliveryTime = item.DeliveryTime; }
                    if (count == 4) { _4thhalf = Convert.ToDateTime(item.EntryTime); _4thDeliveryTime = item.DeliveryTime; }
                    if (count == 5) { _5thHalf = Convert.ToDateTime(item.EntryTime); _5thDeliveryTime = item.DeliveryTime; }
                    if (count == 6) { _6thHalf = Convert.ToDateTime(item.EntryTime); _6thDeliveryTime = item.DeliveryTime; }
                    if (count == 7) { _7thhalf = Convert.ToDateTime(item.EntryTime); _7thDeliveryTime = item.DeliveryTime; }
                    if (count == 8) { _8thHalf = Convert.ToDateTime(item.EntryTime); _8thDeliveryTime = item.DeliveryTime; }


                    count += 1;
                }

                dtpDeliveryDate.Tag = selectedtm;
                txtDeliveryTime.Tag = td;
        ...
        private void ctlMediaSearchControl_ItemSelected(SearchResultListControl<BusinessMedia> sender, BusinessMedia item)
        {
            ...
            if (dgTests.Rows.Count > 0) CalculateMediaCommission(0);
        }
        ...
        private void ctrl_LostFocus(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtMedia.Text)) txtMedia.Tag = null;
        }
        ...
        public void ctlTestSearch_ItemSelected(Controls.SearchResultListControl<Model.TestItem> sender, Model.TestItem item)
        {
            ...
            new TestService().PopulateSelectedTestData(_SelectedTests, _tItem, txtTestCode.Text, dtpDeliveryDate.Value, txtDeliveryTime.Text);
            FillTestGrid();

            if (txtRegNo.Tag != null)
            {
                SetRegHolderDiscount((RegRecord)txtRegNo.Tag);
            }

            if (txtMedia.Tag != null)
            {
                CalculateMediaCommission(0);
            }

            if (_tItem.IsPackage)
            {
                if (_tItem.PackageDiscount > 0) txtDiscount.Text = _tItem.PackageDiscount.ToString();
                txtPatientName.Tag = _tItem.TestId; // as Package Id
            }
        ...
        private void SetRegHolderDiscount(RegRecord _regRecord)
        {
            RegDiscountPlan _discountPlan = new CommonService().GetRegDiscountPlan(_regRecord.CompanyId, _regRecord.DesignationId);
            ...
            {
                // LabTotal
                double labtotal = _SelectedTests.Where(item => item.ReportTypeId != 63 && item.DiscountPlanGroup == "Lab").Sum(item => item.Cost);
                double Nonlabtotal = _SelectedTests.Where(item => item.ReportTypeId != 63 && item.DiscountPlanGroup == "RIM").Sum(item => item.Cost);

                labLess = (labtotal * _discountPlan.LabDiscount) / 100;
                NonlabLess = (Nonlabtotal * _discountPlan.NonLabDiscount) / 100;
                totalLess = labLess + NonlabLess;
        ...
        private void dgTests_UserDeletedRow(object sender, DataGridViewRowEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    // get Media Type 
                    int mediaId = (int)txtMedia.Tag;
                    TestItem tesetItem = txtTestCode.Tag as TestItem;


                    //List<MediaCategoryReportTypeCommission> mediaComission = new TestService().GetReportTypeByCategoryId(0, mediaId, tesetItem.ReportTypeId);

                    //double deletTest = 0;
                    //double discountMinus = 0;

                    //foreach (var mm in mediaComission)
                    //{
                    //    if(mm.IsPercent == true && _SelectedItem.ReportTypeId == mm.ReportTypeId)
                    //    {
                    //        deletTest = (_SelectedItem.Cost * mm.CommissionPercent )/ 100;
                    //    } else if(mm.IsPercent == false && _SelectedItem.ReportTypeId == mm.ReportTypeId)
                    //    {
                    //        deletTest = mm.Commission;
                    //    }

                    //    // double.TryParse(lblPcDiscount.Text, out  discountMinus);
                    //  // 
                    //}

                    //discountMinus = totalMediaCommTk - deletTest;
                    //totalMediaCommTk -=discountMinus;

                    //  MessageBox.Show(deletTest+ " ");

                    CalculateMediaCommission(_SelectedItem.Cost);
                    // lblPcDiscount.Text = (discountMinus).ToString();
        ...
        private async void HandleNewTestAddedActivity(long _PatientId, double newtestcost, double newdiscount, double newpaidtk, double _balance)
        {
            long _regNo = 0;
            long _RxId = 0;
            ...
            if (String.IsNullOrEmpty(_message))
            {
                if (result == DialogResult.Yes)
                {
                    ...
                    await ShowCashMemo(_PatientId);
                    // await ShowCashMemo2(_PatientId);
        ...
        private async Task<bool> ShowCashMemo2(long _PatientId)
        {
            int _index = 0;
            bool isDue = false;
        ...
        private async void AddNewPatient()
        {
            long _regNo = 0;
            long _billNo = 0;
            long _RxId = 0;
            ...
            int PkgId = 0;
            double drDiscount = 0;
            double.TryParse(txtDrDisTk.Text, out drDiscount);


            if (txtPatientName.Tag != null)
            {
                int.TryParse(txtPatientName.Tag.ToString(), out PkgId);
            }
            ...
            double pcDiscount = 0;
            double.TryParse(lblPcDiscount.Text, out pcDiscount);
            // BusinessMedia business = txtMedia.Tag as BusinessMedia;

            //if (discount > pcDiscount && txtMedia.Tag != null)
            //{
            //    MessageBox.Show("Error! Discount can not be more than " + pcDiscount.ToString());
            //    return;
            //}
            ...
            double _cardormobileServiceChargeTk;
            double.TryParse(txtCardOrMobileCharge.Text, out _cardormobileServiceChargeTk);

            double _dueAmount = _testAmount - (cashpaidTk + _cardpaymentTk + _mobilePaymentTk + discount);
            ...
            VMDiagEndPointDataCapture voucherObj = new VMDiagEndPointDataCapture();
            voucherObj.SaleAmount = _testAmount;
            voucherObj.IPDSaleAccountId = 11;
            voucherObj.OPDSaleAccountId = 12;
            voucherObj.IPDCashAccId = 1976;
            voucherObj.OPDCashAccId = 1977;
            voucherObj.IPDDiscountAccId = 1974;
            voucherObj.OPDDiscountAccId = 1975;
            voucherObj.ReceivableAccId = 1967;
            voucherObj.DiscountAmount = discount;
            voucherObj.DrDiscountAmount = drDiscount;
            voucherObj.CashPaidAmount = cashpaidTk;
            voucherObj.CardPayment = _cardpaymentTk;
            voucherObj.MobilePayment = _mobilePaymentTk;
            voucherObj.CardOrMobileServiceCharge = _cardormobileServiceChargeTk;
            voucherObj.DueAmount = _dueAmount;
            ...
            voucherObj.MediaId = 0;
            voucherObj.discountCardNo = _discountCardNo;
            voucherObj.WorkStationId = MainForm.WorkStationId;

            if (txtMedia.Tag != null)
            {
                _MediaId = Convert.ToInt32(txtMedia.Tag);
                voucherObj.MediaId = _MediaId;
            }
            int marketingid = 0;
            if (txtMarketing.Tag != null)
            {
                marketingid = Convert.ToInt32(txtMarketing.Tag);
            }
            ...
            if (_admissionNo > 0)
            {
                voucherObj.ReceivableAccId = 1966;

                HospitalPatientInfo _pInfo = new HospitalService().GetHospitalPatientInfoById(_admissionNo);

                voucherObj.HospitalPatientInfo = _pInfo;
                voucherObj.IsIpdSale = true;
            ...
            if (String.IsNullOrEmpty(_message))
            {
                ...
                try
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (txtRxId.Tag != null)
                        {
                            RxVisitHistory _rx = txtRxId.Tag as RxVisitHistory;
                            _RxId = _rx.RxVisitId;
                        }
                        ...
                        patient.MarketingId = marketingid;
                        patient.DiscountCareOf = txtCareOf.Text;
                        ...
                        patient.SampleCollectionStatus = SampleCollectionStatusEnum.NC.ToString();
                        patient.Isfree = false;
                        ...
                        foreach (DataGridViewRow row in dgTests.Rows)
                        {
                            ...
                            tCost.Qty = qty;
                            tCost.Cost = selectedTests.Cost;
                            //tCost.DiscountInPercent = selectedTests.discountInPercent;
                            //tCost.Discount = selectedTests.discount;
                        ...
                        voucherObj.testsConducted = testsConducted;


                        Patient _patient = await new DiagFinancialService().SaveAndCommitNewSale(patient, testsCost, voucherObj, MainForm.orgSetting.IsIntegratedAccountInAction);
                        ...
                        if (_patient.PatientId > 0)
                        {
                            ...
                            await ShowCashMemo(_patient.PatientId);
                            await ShowCashMemo2(_patient.PatientId);
                            await ShowCashMemo3(_patient.PatientId);

                            //List<VMGroupName> _groupList = new TestService().GetDistinctGroup(_patient.PatientId);

                            //foreach (var item in _groupList)
                            //{
                            //    ShowToekn(_patient.PatientId, item.GroupName);
                            //    Thread.Sleep(100);
                            //}
        ...
        private void ShowToekn(long _pId, string GroupName)
        {
            ds = new DataSet();
            ds = new ReportService().GetDiagToken(_pId, GroupName);


            ReportViewer rv = new ReportViewer();

            rptToken _rpt = new rptToken();
        ...
        private async Task<bool> ShowCashMemo3(long _PatientId)
        {
            int _index = 0;
            bool isDue = false;
            bool isDiscounted = false;
        ...
        private RegRecord GetRegNoLong()
        {
            ...
            else
            {
                ...
                if (cmbDistricts.Text != "Select District")
                {
                    _rgRecord.District = cmbDistricts.Text;
                }
                //else
                //{
                //    MessageBox.Show("Place Select Distic");
                //    return null;
                //}

                if (cmbAreaOrThana.Text != "Select Area/Thana")
                {
                    _rgRecord.ArearOrThana = cmbAreaOrThana.Text;
                }
                //else
                //{
                //    MessageBox.Show("Place Select Thana");
                //    return null;
                //}
        ...
        private string GetDeliveryTime()
        {
            ...
            ReportDeliveryTimingMaster tm = dtpDeliveryDate.Tag as ReportDeliveryTimingMaster;
            ...
            else
            {
                ...
                if (isReportDeliveryWeekEndSchedule)
                {
                    //DateTime _t1 = Utils.GetServerDateAndTime();
                    DateTime dt = Utils.GetServerDateAndTime(); //Will be the current date APR/01/2016
                    ...
                    if (str.Equals("Thursday"))
                    {
                        ...
                        dtpDeliveryDate.Value = DateTime.Now.AddDays(1);
                        return _1stDeliveryTime;
                    }
                    else if (str.Equals("Friday"))
                    {
                        //Regular Report Timing Records
                        ReportDeliveryTimingMaster rrdtm = new TestService().GetRegularReportDeliveryTimeMaster();
                        List<ReportDeliveryTimingDetail> td = new TestService().GetReportDeliveryTimingDetail(rrdtm.RDTMId);
                        ReportDeliveryTimingDetail tdObj = td.FirstOrDefault();

                        if (tm.TotalDeliverySlot == 1)
                        {

                            if (_t1.TimeOfDay.Ticks <= _firsthalf.TimeOfDay.Ticks) return _1stDeliveryTime;

                            if (_t1.TimeOfDay.Ticks > _firsthalf.TimeOfDay.Ticks)
                            {

                                dtpDeliveryDate.Value = DateTime.Now.AddDays(1);
                                return tdObj.DeliveryTime;

                            }
                        }

                        if (tm.TotalDeliverySlot == 2)
                        {

                            if (_t1.TimeOfDay.Ticks <= _firsthalf.TimeOfDay.Ticks) return _1stDeliveryTime;

                            if (_t1.TimeOfDay.Ticks > _firsthalf.TimeOfDay.Ticks && _t1.TimeOfDay.Ticks <= _secondHalf.TimeOfDay.Ticks) return _2ndDeliveryTime;


                            if (_t1.TimeOfDay.Ticks > _secondHalf.TimeOfDay.Ticks)
                            { //&& _t1.TimeOfDay.Ticks <= _firsthalf.TimeOfDay.Ticks) {
                        ...
                        if (tm.TotalDeliverySlot == 3)
                        {
                            ...
                            if (_t1.TimeOfDay.Ticks > _secondHalf.TimeOfDay.Ticks && _t1.TimeOfDay.Ticks <= _thirdHalf.TimeOfDay.Ticks) return _3rdDeliveryTime;


                            if (_t1.TimeOfDay.Ticks > _thirdHalf.TimeOfDay.Ticks)
                            { //&& _t1.TimeOfDay.Ticks <= _firsthalf.TimeOfDay.Ticks) {
                ...
                else if (tm.TotalDeliverySlot == 1)
                {
                ...
                if (tm.TotalDeliverySlot == 4)
                {
                    ...
                    if (_t1.TimeOfDay.Ticks > _thirdHalf.TimeOfDay.Ticks && _t1.TimeOfDay.Ticks <= _4thhalf.TimeOfDay.Ticks) return _4thDeliveryTime;

                    if (_t1.TimeOfDay.Ticks > _4thhalf.TimeOfDay.Ticks)
                    { //&& _t1.TimeOfDay.Ticks <= _firsthalf.TimeOfDay.Ticks) {
                ...
                if (tm.TotalDeliverySlot == 5)
                {
                    ...
                    if (_t1.TimeOfDay.Ticks > _4thhalf.TimeOfDay.Ticks && _t1.TimeOfDay.Ticks <= _5thHalf.TimeOfDay.Ticks) return _5thDeliveryTime;


                    if (_t1.TimeOfDay.Ticks > _5thHalf.TimeOfDay.Ticks)
                    { //&& _t1.TimeOfDay.Ticks <= _firsthalf.TimeOfDay.Ticks) {
                ...
                if (tm.TotalDeliverySlot == 6)
                {
                    ...
                    if (_t1.TimeOfDay.Ticks > _5thHalf.TimeOfDay.Ticks && _t1.TimeOfDay.Ticks <= _6thHalf.TimeOfDay.Ticks) return _6thDeliveryTime;

                    if (_t1.TimeOfDay.Ticks > _6thHalf.TimeOfDay.Ticks)
                    { //&& _t1.TimeOfDay.Ticks <= _firsthalf.TimeOfDay.Ticks) {
                ...
                if (tm.TotalDeliverySlot == 7)
                {
                    ...
                    if (_t1.TimeOfDay.Ticks > _6thHalf.TimeOfDay.Ticks && _t1.TimeOfDay.Ticks <= _7thhalf.TimeOfDay.Ticks) return _7thDeliveryTime;

                    if (_t1.TimeOfDay.Ticks > _7thhalf.TimeOfDay.Ticks)
                    { //&& _t1.TimeOfDay.Ticks <= _firsthalf.TimeOfDay.Ticks) {
                ...
                if (tm.TotalDeliverySlot == 8)
                {
                    ...
                    if (_t1.TimeOfDay.Ticks > _7thhalf.TimeOfDay.Ticks && _t1.TimeOfDay.Ticks <= _8thHalf.TimeOfDay.Ticks) return _8thDeliveryTime;


                    if (_t1.TimeOfDay.Ticks > _8thHalf.TimeOfDay.Ticks)
                    { //&& _t1.TimeOfDay.Ticks <= _firsthalf.TimeOfDay.Ticks) {
        ...
        private async Task<bool> ShowCashMemo(long _PId)
        {
        ...
        {
            double _discount = 0, _drDiscount = 0, _cashpaidtk = 0, _cardormobilepaid = 0;

            double.TryParse(txtDiscount.Text, out _discount);

            double.TryParse(txtDrDisTk.Text, out _drDiscount);

            double.TryParse(txtPaid.Text, out _cashpaidtk);
            double.TryParse(txtCardOrMobileReceiveTk.Text, out _cardormobilepaid);

            double _totalDeduction = _discount + _drDiscount + _cashpaidtk + _cardormobilepaid;
        ...
        private void ClearFields()
        {
            ...
            dtpDeliveryDate.Value = Utils.GetServerDateAndTime();
            txtDeliveryTime.Text = GetDeliveryTime();
            dgTests.Rows.Clear();
            txtCellPhone.Focus();
            unlocked = true;
        ...
        private async void txtPatientName_Leave(object sender, EventArgs e)
        {
            ...
            //Load Report Delivery Schedule Master
            DateTime dt = Utils.GetServerDateAndTime(); //Will be the current date APR/01/2016
            string str = dt.ToString("dddd");

            ReportDeliveryTimingMaster selectedtm = await new TestService().GetSelectedReportDeliveryTimeMaster();

            ReportDeliveryTimingMaster _tm = new TestService().GetWeekEndReportDeliveryTimingMaster();
            if (str.Equals("Thursday"))
            {
                List<ReportDeliveryTimingDetail> _td = new TestService().GetReportDeliveryTimingDetail(_tm.RDTMId);
                DateTime _schduleStartTime = new CalculateCabinCharge().CombinedDateAndTimePart(Utils.GetServerDateAndTime().Date, _tm.WeekEndStartTime);
                if (Utils.GetServerDateAndTime() >= _schduleStartTime)
                {
                    isReportDeliveryWeekEndSchedule = true;


                    int count = 1;
                    foreach (var item in _td)
                    {
                        if (count == 1) { _firsthalf = Convert.ToDateTime(item.EntryTime); _1stDeliveryTime = item.DeliveryTime; }
                        if (count == 2) { _secondHalf = Convert.ToDateTime(item.EntryTime); _2ndDeliveryTime = item.DeliveryTime; }
                        if (count == 3) { _thirdHalf = Convert.ToDateTime(item.EntryTime); _3rdDeliveryTime = item.DeliveryTime; }
            ...
            {
                dtpDeliveryDate.Tag = selectedtm;
                isReportDeliveryWeekEndSchedule = false;
                List<ReportDeliveryTimingDetail> _td = new TestService().GetReportDeliveryTimingDetail(selectedtm.RDTMId);
            ...
            else if (txtPatientName.Text.Substring(0, 2).ToLower() == "md")
            {
                cmbGender.Text = "Male";
            }

            else if (txtPatientName.Text.Length > 2 && txtPatientName.Text.Substring(0, 4).ToLower() == "miss")
            {
                cmbGender.Text = "Female";
            }
            else if (txtPatientName.Text.Length > 2 && txtPatientName.Text.Substring(0, 3).ToLower() == "mst")
            {
                cmbGender.Text = "Female";
            }
        ...
        private async void txtRxId_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                long _RxIN = 0;
                ...
                if (rxVisitHistory != null)
                {
                    ...
                    RxPatientMasterData pm = await new RxService().GetRxMasterDataByPMIdAsync(rxVisitHistory.RxPMId);

                    unlocked = false;
                    RegRecord _record = new RegRecordService().GetRegRecordByRegNo(pm.RegNo);
                    txtRegNo.Text = pm.RegNo.ToString();
                    unlocked = true;
                    ...
                    if (_record != null)
                    {
                        ...
                        cmbGender.Text = _record.Sex;
                        ChamberPractitioner cp = new DoctorService().GetChamberPractitionerById(rxVisitHistory.CpId);

                        Doctor _doctor = new DoctorService().GetDoctorById(cp.DoctorId);
                        unlocked = false;
                        if (_doctor != null)
                        {
                        ...
                        {
                            txtRefBy.Text = "Not Found";
                            txtRefBy.Tag = null;
                        }

                        unlocked = true;

                        List<RxTest> _listAdvicedTests = new RxService().GetRxTestsByRxId(rxVisitHistory.RxVisitId);
                        _SelectedTests = new List<SelectedTestItemsForPatient>();
                        foreach (var item in _listAdvicedTests)
                        {
                            ...
                            new TestService().PopulateSelectedTestData(_SelectedTests, titem, titem.TestCode.ToString(), dtpDeliveryDate.Value, txtDeliveryTime.Text);
        ...
        private async void LoadPrevoiusPatientInfo(long _PId)
        {
            ...
            else
            {
                ...
                txtCareOf.Text = _PatientInfo.DiscountCareOf;

                IsNewEntry = false;

                txtBillNo.Tag = _PatientInfo; //This will be checked for either new patient or old patient

                txtRegNo.Text = _PatientInfo.PatientId.ToString();
        ...
        private void btnPrevId_Click(object sender, EventArgs e)
        {
            ...
            if (_patient != null)
            {
                txtPrevBill.Text = _patient.PatientId.ToString();
        ...
        {
            //if (!String.IsNullOrWhiteSpace(txtRegNo.Text) && !CalledFromOtherPlace)
            //{
            //    ctrlRegRecordSearch.Visible = true;
            //    ctrlRegRecordSearch.LoadDataByType(txtRegNo.Text);
            //    ctrlRegRecordSearch.txtSearch.Text = txtRegNo.Text;
            //    ctrlRegRecordSearch.txtSearch.SelectionStart = ctrlRegRecordSearch.txtSearch.Text.Length;
            //    ctrlRegRecordSearch.txtSearch.SelectionLength = 0;
            //}
        }
        ...
        {
            pnlPatient.Size = new Size(1410, 696);

            pnlPatient.Location = new Point(0, 0);
        ...
        {
            pnlPatient.Location = new Point(pnlPatient.Location.X + 1270, 0);
        ...
        private async void btnDiagShow_Click(object sender, EventArgs e)
        {
            List<VMDiagPatientBasicInfo> _PDiaglist = await LoadPatients();
            dgPatient.Tag = _PDiaglist;
            lblTotalPatient.Text = _PDiaglist.Count.ToString();
            FillListGrid(_PDiaglist);
        }

        private async void txtPatietNid_TextChanged(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtPatietNid.Text))
            {
                await LoadPatients();
            }
            else
            {
                _regList = _regList.Where(x => x.PatientId.ToString().ToLower().Contains(txtPatietNid.Text.ToLower())).ToList();
                FillListGrid(_regList);

            }
        }

        private async void txtPatientBillNo_TextChanged(object sender, EventArgs e)
        {
            List<VMDiagPatientBasicInfo> _regList = dgPatient.Tag as List<VMDiagPatientBasicInfo>;
            ...
            if (string.IsNullOrWhiteSpace(txtPatientBillNo.Text))
            {
                await LoadPatients();
            }
            else
            {
                _regList = _regList.Where(x => x.MediaName.ToString().ToLower().Contains(txtPatientBillNo.Text.ToLower())).ToList();
                FillListGrid(_regList);
        ...
        private void LoadPatientsDatabySearchParam(string _searchParam, string _paramType)
        {
            if (dgPatient.Tag != null)
            {
                ...
                {
                    _regList = _regList.Where(x => x.PatientName.ToLower().Contains(_searchParam.ToLower())).ToList();

                    FillListGrid(_regList);
                }

                else if (_paramType == "Ref By")
                {
                    _regList = _regList.Where(x => x.Name.ToLower().Contains(_searchParam.ToLower())).ToList();
                    FillListGrid(_regList);
                }
        ...
        {
            if (string.IsNullOrEmpty(txtSearchByMobile.Text.ToLower()) || txtSearchByMobile.Text.Trim() == "Search by Mobile No")
            {
                // LoadPatientsDatabyName(txtAdmId.Text, "admid");
            }
            else
        ...
        private void dgPatient_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (dgPatient.SelectedRows.Count != 0)
            {
                ...
                dgPatient.Tag = _pInfo;

                List<PatientLedger> _ledgerVerify = new PatientLedgerService().GetLedgerByPatientId(_pInfo.PatientId);
                List<TestsCost> _testCostVerify = new PatientService().GetTestList(_pInfo.PatientId);

                if (_ledgerVerify.Count > 0 && _testCostVerify.Count > 0)
                {
                    ShowCashMemoDeshBorad(_pInfo);
        ...
        private async void ShowCashMemoDeshBorad(VMDiagPatientBasicInfo _Pinfo)
        {
            ...
            string p1 = string.Empty, p2 = string.Empty, p3 = string.Empty;
            ds = new DataSet();
            ds = await new ReportService().GetCashMemo(_Pinfo.PatientId);
            ...
            if (String.IsNullOrEmpty(_remarks)) _remarks = _Pinfo.DiscountCareOf;

            List<TestGroup> _tGroups = new PatientService().GetTestGroupsByPatientId(_Pinfo.PatientId);
            ...
            // _cashmemo.SetParameterValue("Title", cmbTitle.Text);
        ...
        {
            frmAddReferral _frm = new frmAddReferral();
            _frm.Show();
        }

        private void btnAddMedia_Click(object sender, EventArgs e)
        {
            frmAddMedia frm = new frmAddMedia();
            frm.Show();
        }
        ...
        private void txtMarketing_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Space)
            {
                ctlMarketing.Visible = true;
                ctlMarketing.LoadDataByType("Marketing");
                txtTestCode.Focus();
            }
        }

        private void ctlMarketing_SearchEsacaped(bool IsEscaped)
        {
            if (IsEscaped)
                txtMarketing.Focus();
        }
        ...
        private void txtDrDisper_TextChanged(object sender, EventArgs e)
        {
            if (IsNewEntry)
            {
                double lessInPercent = 0, totalLess = 0;

                double.TryParse(txtDrDisper.Text, out lessInPercent);
                ...
                if (total > 0)
                {
                    if (txtBillNo.Tag == null)  // Non-update mode
                    {
                        totalLess = (total * lessInPercent) / 100;
                        txtGrandTotal.Text = ((total + _vacuTotal) - totalLess).ToString();
                        txtDrDisTk.Text = totalLess.ToString();
                    }
                    else   //Update mode
                    {
                        double _total = 0;
                        ...
                        double _prevdiscount = new PatientService().GetDrDiscountAmount(_PatientId);
                        ...
                        double _newdiscount = (_newtotal * lessInPercent) / 100;
                        double _totalLess = _prevdiscount + _newdiscount;
                        txtDrDisTk.Text = _totalLess.ToString();
                        txtGrandTotal.Text = (_total - _totalLess).ToString();
        ...
        private void txtDrDisTk_TextChanged(object sender, EventArgs e)
        {
            if (IsNewEntry)
            {
                double total = _SelectedTests.Where(x => x.ReportTypeId != 63).Sum(y => y.Cost);
            ...
            else
            {
                ...
                double _disc = 0;
                double.TryParse(txtDrDisTk.Text, out _disc);
        ...
        private void dgTests_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 5)
            {
                ...
                double rate = 0;
                int[] disArr = new int[2];


                //if (dgTests.Rows[e.RowIndex].Cells[5].Value == null) return;
                //  string discountInToParcent = dgTests.Rows[e.RowIndex].Cells[5].Value.ToString();
                if (dgTests.Rows[e.RowIndex].Cells[5].Value != null)
                {
                    double.TryParse(dgTests.Rows[e.RowIndex].Cells[4].Value.ToString(), out rate);
                    double.TryParse(dgTests.Rows[e.RowIndex].Cells[5].Value.ToString(), out discPercent);
                }


                if (discPercent == 0 || dgTests.Rows[e.RowIndex].Cells[5].Value == null)
                {
                    double.TryParse(txtDiscount.Text, out double previousDiscount);
                    double.TryParse(dgTests.Rows[e.RowIndex].Cells[6].Value.ToString(), out double testDiscount);
                    txtDiscount.Text = (previousDiscount - testDiscount).ToString();

                    dgTests.Rows[e.RowIndex].Cells[6].Value = "";


                    return;
                }
                dgTests.Rows[e.RowIndex].Cells[6].Value = rate * discPercent / 100;
            }
        ...
        private void ctlPatientSearch_Load(object sender, EventArgs e)
        {

        }

        private void ctlPatientSearch_SearchEsacaped_1(bool IsEscaped)
        {
            if (IsEscaped)
            {
                ctlPatientSearch.Visible = false;
        ...
        private void btnMediaPayment_Click(object sender, EventArgs e)
        {
            // frmMediaPayment payment = new frmMediaPayment();
            //payment.Show();
        }

        private void placeHolderTextBox1_TextChanged(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(txtRefDoc.Text.ToLower()) || txtRefDoc.Text.Trim() == "Search by Ref By")
            {
                // LoadPatientsDatabyName(txtAdmId.Text, "admid");
            }
            else
            {
                ...
                LoadPatientsDatabySearchParam(txtRefDoc.Text, "Ref By");
            }
        }
        ...
        private void ctlTestSearch_SearchEsacaped_1(bool IsEscaped)
        {
            if (IsEscaped)
            {
                txtTestCode.Focus();
        ...
        private void ctlDoctorSearch_SearchEsacaped_1(bool IsEscaped)
        {
            if (IsEscaped)
            {
                txtRefBy.Focus();
        ...
        private void ctlMediaSearchControl_SearchEsacaped_1(bool IsEscaped)
        {
            if (IsEscaped)
            {
                txtMedia.Focus();
        ...
        private void CalculateMediaCommission(double deleteTestItem)
        {
            // get Media Type 
            int mediaId = (int)txtMedia.Tag;
            double totalMediaCommTk = 0;

            //  int MediaCategoryId = new TestService().GetCategoryIdByMediaId(mediaId);

            // get Mediay category 
            // string meidaCategoryNam = new TestService().GetMediaCategoryName(MediaCategoryId);

            // Get Media Commission Retport Type 
            TestItem tesetItem = txtTestCode.Tag as TestItem;
            //List< MediaCategoryReportTypeCommission> mediaComission = new TestService().GetReportTypeByCategoryId(MediaCategoryId, mediaId, tesetItem.ReportTypeId);
            ...
            foreach (DataGridViewRow row in dgTests.Rows)
            {
                ...
                SelectedTestItemsForPatient test = row.Tag as SelectedTestItemsForPatient;
                if (test == null) continue;


                TestItem testItem = new TestService().GetTestItemByTestCode(test.TestCode);
                if (testItem == null) continue;

                foreach (MediaCategoryReportTypeCommission m in mediaComission)
                {

                    if (m.IsPercent == true && m.CommissionPercent > 0 && test.ReportTypeId == m.ReportTypeId && test.ReportTypeId != 63)
                    {
                        totalMediaCommTk += (testItem.Rate * m.CommissionPercent) / 100;

                    }
                    else if (m.IsPercent == false && m.Commission > 0 && test.ReportTypeId == m.ReportTypeId && test.ReportTypeId != 63)
                    {
                        totalMediaCommTk += m.Commission;
                    }
                }
                // totalMediaCommTk += testItem.MediaComm;
            }

            lblPcDiscount.Text = totalMediaCommTk.ToString();
        }
    }
}
