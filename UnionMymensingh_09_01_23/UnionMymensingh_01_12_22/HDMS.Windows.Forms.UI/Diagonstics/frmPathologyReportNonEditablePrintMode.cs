...
{
    public partial class frmPathologyReportNonEditablePrintMode : Form
    {
        ...
        public frmPathologyReportNonEditablePrintMode()
        {
            InitializeComponent();
        }
        ...
        private async Task GoAndHuntDiagReports()
        {
            ...
            if (_PatientInfo == null)
            {
                ...
                _PatientInfo = new PatientService().GetPatientByBillNo(_billNo);
            ...
            //  await new LabService().MAP_LIS_RESULT_WITH_SOFTWARE_REPORT_DEFINATION(txtBillNo.Text);

            //  await Load_LIS_Result(txtBillNo.Text, _PatientInfo);
        ...
        private async Task<bool> Load_LIS_Result(string _billNo, Patient _PatientInfo)
        {
           return  await  new LabService().LoadLISMachineData(_billNo, _PatientInfo);
        }
            ...
            foreach (VMReportDefination item in _rItems)
            {
                ...
                else
                {
                    if(item.TestDetailId == 1053 || item.TestDetailId == 1066)
                    {
                        ...
                        row.CreateCells(gvTestsDetails, item.TestTitle, _nvalStr, item.ResultUnit, item.NormalValue, item.GroupTitle, item.TestDetailId);
                    }
                    else
                    {
                        ...
                        row.CreateCells(gvTestsDetails, item.TestTitle, _result, item.ResultUnit, item.NormalValue, item.GroupTitle, item.TestDetailId);
        ...
        private void ViewUrineReport(long reportId)
        {
            ...
            rpturineVW3000 _urinereport = new rpturineVW3000();
        ...
        private async void TV_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (IsTreePopulated)
            {
                ...
                if (arr.Length > 1)  // By Report Type
                {
                   ...
                   await LoadTestResultByReportType(arr[0]);
                    isListReset = false;
                ...
                else   // By Test Item
                {
                   ...
                   await LoadTestResultByTestId(_testId);

                }
            }
        }

        private async Task<bool> LoadTestResultByTestId(int _testId)
        {
            ...
            if (_testItem == null)  return await Task.FromResult(true); ;
        ...
        private void FillTestGridForPreviousReport()
        {
            ...
            foreach (VMReportDefination item in _SelectedTestItemsForPathologyReport)
            {
                ...
                row.CreateCells(gvTestsDetails, item.TestTitle, item.TestResult, item.MachineResult, item.ResultUnit, item.NormalValue);
        ...
        private async Task<bool> LoadTestResultByReportType(string _reportTypeId)
        {
            ...
            List<VMReportDefination> _testList = new List<VMReportDefination>();
                ...
                if (_testList.Count == 0) return await Task.FromResult(true); 
        ...
        private async void LoadLISTestResultByReportTypeAndDevice(int reportTypeId, string _deviceCode)
        {
            _SelectedTestItemsForPathologyReport = new List<VMReportDefination>();
            gvTestsDetails.SuspendLayout();
            gvTestsDetails.Rows.Clear();
        ...
        private void FillTestGrid()
        {
            ...
            foreach (VMReportDefination item in _SelectedTestItemsForPathologyReport)
            {
                ...
                if (item.TestName.ToLower() == "urine r/e" || item.TestName.ToLower() == "stool r/e")
                {
                    row.CreateCells(gvTestsDetails, item.TestTitle, item.NormalValue, item.MachineResult, item.ResultUnit, item.NormalValue, item.GroupTitle, item.TestDetailId);
                }
                else
                {
        ...
        {
            if (tabPage1.Tag != null && tabControl1.Tag != null && IsPathMachineLoaded) // This tag not null means current result displayed result comes from LIS
            {
                // tabControl1.Tag contains ReportTypeId

                int _reportTypeId = 0;
                int.TryParse(tabControl1.Tag.ToString(), out _reportTypeId);

                PathologicalMachine _machine = (PathologicalMachine)cmbMachine.SelectedItem;

                LoadLISTestResultByReportTypeAndDevice(_reportTypeId, _machine.MachineShortName);

            }
        }

        private void CellMachineCopyPrint_Click(object sender, EventArgs e)
        {
            PathologyReport _report = new PathologyReport();
            ...
            if (txtBillNo.Tag != null)
            {
                ...
                try
                {
                    ...
                    _report.ReportDate = Utils.GetServerDateAndTime();
                    _report.ReportTime = Utils.GetServerDateAndTime().ToString("hh:mm tt");
                    _report.ReportDoctor = 0;
                    _report.ReportTechnologist = 0;
                    _report.PreparedBy = lblPreparedBy.Text;
                    _report.ReportType = _reportType;
                    _report.AnyComments = txtAnyComments.Text;
                    _report.ModifiedDate = Utils.GetServerDateAndTime();
                    ...
                    if (_pr.Id > 0)
                    {
                        ...
                        SaveAndPrintReportForCELLDYN(_pr);
        ...
        private void SaveAndPrintReportForCELLDYN(PathologyReport _report)
        {
            bool isDynamicReport = false;
            string _staticvalues = string.Empty;
            string _staticfields = string.Empty;
            ...
            if (isDynamicReport)
            {
                ...
                ViewReportCellDYN(_report.Id);

            }
        }

        private void ViewReportCellDYN(long id)
        {
            string _identity5 = string.Empty;
            string _identity3 = string.Empty;
            string _comments = string.Empty;
            ...
            RptPathologyCellDynFormat _pathreport = new RptPathologyCellDynFormat();
            ...
            if (cmbPathologist.Tag != null)
            {
                ...
                _pathreport.DataDefinition.FormulaFields[7].Text = "'" + "Machine Copy" + "'";
                _pathreport.DataDefinition.FormulaFields[8].Text = "'" + "Machine Copy" + "'";
                _pathreport.DataDefinition.FormulaFields[9].Text = "'" + "" + "'";
                _pathreport.DataDefinition.FormulaFields[10].Text = "'" + "" + "'";
                //_pathreport.ParameterFields[10].CurrentValues.Add(_consultant.DIdentityLine3);
                _pathreport.DataDefinition.FormulaFields[11].Text = "'" + "" + "'";
                _pathreport.DataDefinition.FormulaFields[12].Text = "'" + "" + "'";
                _pathreport.DataDefinition.FormulaFields[20].Text = "'" + "" + "'";
            }

            if (cmbTechnologist.Tag != null)
            {
                ReportTechnologist _technologist = (ReportTechnologist)cmbTechnologist.Tag;
                _pathreport.DataDefinition.FormulaFields[13].Text = "'" + "Machine Copy" + "'";
                _pathreport.DataDefinition.FormulaFields[14].Text = "'" + "" + "'";
                _pathreport.DataDefinition.FormulaFields[15].Text = "'" + "" + "'";
                _pathreport.DataDefinition.FormulaFields[16].Text = "'" + "" + "'";
            }
        ...
        private void txtHpBillNo_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                long _hpbill = 0;
                long.TryParse(txtHpBillNo.Text, out _hpbill);

               List<Patient> _pBills = new PatientService().GetDiagPatientsByHpBillNo(_hpbill);
        ...
        private async void dgBillIds_RowHeaderMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
