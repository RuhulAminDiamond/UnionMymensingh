...
namespace HDMS.Windows.Forms.UI.Hospital
{
    public partial class frmOPDServiceHeadEntry : Form
    {
        ...
        {
            List<VMOPDServiceHead> _shList = new HospitalService().GetAllOPDServiceHeads();
            FileServiceHeadGridList(_shList);

            

        }

        private void FileServiceHeadGridList(List<VMOPDServiceHead> _shList)
        {
           
            foreach(var item in _shList)
            {
                dgServiceHeads.SuspendLayout();
                //dgServiceHeads.Rows.Clear();
                DataGridViewRow row = new DataGridViewRow();
                row.Height = 30;
                row.CreateCells(dgServiceHeads,item.ServiceHeadId,item.ServiceHeadName,item.Rate);
                //dgServiceHeads.Rows.Add(row);
            }
